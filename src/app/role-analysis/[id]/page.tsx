...
import { ArrowLeft, Search, MapPin, Building2, DollarSign, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function DomainDetailPage() {
  ...
  const hasActiveFilters = searchLocation || searchCompany || searchWageLevel;
  ...
  const fetchJobs = async (page: number = 1) => {
    ...
    try {
      const res = await fetch(
        `/api/domain/${encodeURIComponent(roleName)}?page=${page}&limit=${jobsPerPage}`
      );
  ...
  const filteredJobs = hasActiveFilters
    ? allJobs.filter((job) => {
      ...
      return locationMatch && companyMatch && wageMatch;
    })
    : jobs;
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchLocation, searchCompany, searchWageLevel]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              value={searchCompany}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearchCompany(e.target.value)}
              className="w-full"
            />
          </div>
          ...
          <div className="flex flex-wrap gap-2 pt-2 border-t border-border">
            ...
            {searchCompany && (
              <span className="inline-flex items-center gap-1 px-3 py-1 bg-primary/10 text-primary rounded-full text-sm">
                ...
                {searchCompany}
                <button onClick={() => setSearchCompany("")} className="ml-1 hover:text-primary/80">×</button>
              </span>
            )}
            {searchWageLevel && (
              <span className="inline-flex items-center gap-1 px-3 py-1 bg-primary/10 text-primary rounded-full text-sm">
      ...
      {displayJobs.length > 0 ? (
        ...
        <div className="text-center py-12 bg-card border border-border rounded-lg">
          <p className="text-muted-foreground mb-2">
            {hasActiveFilters ? "No jobs match your search criteria" : "No jobs found for this domain"}
          </p>
          {hasActiveFilters && (
